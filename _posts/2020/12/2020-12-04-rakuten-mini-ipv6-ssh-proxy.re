...
Archer A6はDS-Liteを有効にした状態ではND-Proxy（おそらくパケットフィルタの意を含む）が基本的にONでパススルー設定にできず、ND-Proxyをうまいことオフにしつつ手元機材がIPv6アドレスを取れる状態に持っていけないかと試行錯誤しましたがv6力の低さゆえうまくいきませんでした@<fn>{fn-weird-a6}。
このため、Archer A6のルーター機能は使わないことにしました。

//footnote[fn-weird-a6][WAN側からのpingを拒否しても普通にpingは素通しし続けるとか怪しい挙動をするので、ファームウェアの作り込みが甘いのかもしれません。エミュレータを見る限りだとEU版ならワンチャンありそうな気もするけれどJP版はメニュー少ないし謎]
...
テザリングで端末がぶら下がっていない状態なら普通に3-4日間バッテリーが保ったりします（@<img>{2020-12-04-rakuten-mini-battery}）。

//image[2020-12-04-rakuten-mini-battery][テザリング待ち受け状態で25時間経過時点]{
//}
