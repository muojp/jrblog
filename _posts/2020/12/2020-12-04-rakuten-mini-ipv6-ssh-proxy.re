...
一時期騒がれたNTT本家のひかり電話対応ホームゲートウェイのようなフレッツ網内からインバウンド通信を無制限に通すようなマズい状態を回避できるのは嬉しいのですが、ポリシーがちょっと厳しすぎます。
Archer A6ではND-Proxyが基本的にONでパススルー設定にできず、アドレス取得方法をND-Proxy以外に設定すると手元機材がIPv6アドレスを取れる状態に持っていけなかった（それでいて、WAN側からのpingを拒否しても普通にpingは素通しし続ける、、なぜ、、）ので、Archer A6のルーター機能は使わないことにしました。
