...
また、今回のテストコードを書いている際にpanic関連の挙動についての学びがありました。
文脈としては、不正な長さのデータを投入した際に想定通り@<code>{Err()}が戻ってくることを確認するのに加えて、高水準側（ほぼmainメソッド直下）においては想定通りpanicで実行を止めてくれることを確認したかったので、次のようなコードを書きました
